...
/**
 * This script builds a simple version of the @growly/suite widget.
 */
...
// Paths
const GROWLY_SUITE_SRC = path.resolve(__dirname, '../packages/suite/src');
...
if (!fs.existsSync(GROWLY_SUITE_DIST)) {
  console.log(`Creating directory: ${GROWLY_SUITE_DIST}`);
  fs.mkdirSync(GROWLY_SUITE_DIST, { recursive: true });
}
...
// Simple CSS file
const cssSource = path.join(GROWLY_SUITE_SRC, 'styles/simple-widget.css');
const cssTarget = path.join(GROWLY_SUITE_DIST, 'styles.css');
const cssUniswapTarget = path.join(UNISWAP_PUBLIC_DIR, 'styles.css');

if (fs.existsSync(cssSource)) {
  console.log(`Copying ${cssSource} to ${cssTarget}`);
  fs.copyFileSync(cssSource, cssTarget);

  console.log(`Copying ${cssSource} to ${cssUniswapTarget}`);
  fs.copyFileSync(cssSource, cssUniswapTarget);
} else {
...
const jsTarget = path.join(GROWLY_SUITE_DIST, 'index.js');
const jsUniswapTarget = path.join(UNISWAP_PUBLIC_DIR, 'index.js');

console.log(`Writing JavaScript bundle to ${jsTarget}`);
fs.writeFileSync(jsTarget, jsContent);

console.log(`Writing JavaScript bundle to ${jsUniswapTarget}`);
fs.writeFileSync(jsUniswapTarget, jsContent);

console.log('Successfully built and copied simple widget files!');
