...
type Token = NonNullable<NonNullable<definitions['getTokensV7Response']['tokens']>[0]>['token'];

export function useToken(contractAddress: string, tokenId?: string) {
  return useQuery({
    queryKey: ['token', contractAddress, tokenId],
    queryFn: async () => {
      const qs = tokenId ? `tokens=${contractAddress}:${tokenId}` : `collection=${contractAddress}`;
