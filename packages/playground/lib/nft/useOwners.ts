...
export function useOwners(contractAddress: string, tokenId?: string) {
  const qs = tokenId ? `token=${contractAddress}:${tokenId}` : `collection=${contractAddress}`;
  return useQuery({
    queryKey: ['owners', contractAddress],
    queryFn: async () => {
      const response = await fetch(`https://api-base.reservoir.tools/owners/v2?${qs}&limit=2`, {
        method: 'GET',
        headers: {
          accept: '*/*',
