...
export function useReservoirMintData(contractAddress: string, tokenId?: string) {
  ...
  const stage = useMemo(() => {
    ...
    const stages = contractType === 'ERC721' ? collection?.mintStages : token?.mintStages;
    return stages?.find(
      stage =>
  ...
  return {
    ...
    mimeType: (token?.metadata?.mediaMimeType ?? token?.metadata?.imageMimeType ?? '') as string,
    ...
    isEligibleToMint: collection?.isMinting && hasMintingStarted && !hasMintingEnded,
    totalOwners: collection?.ownerCount,
    recentOwners: owners?.owners?.map(owner => owner.address as `0x${string}`),
  };
}
