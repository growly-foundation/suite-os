...
import { CheckoutTypes } from '@/types/appstack';
import { useContext } from 'react';
import { useState } from 'react';
import { AppContext } from '../AppProvider';

export function CheckoutOptions() {
  const { checkoutTypes, setCheckoutTypes, checkoutOptions, setCheckoutOptions } =
    useContext(AppContext);

  const [productId, setProductId] = useState<string>(checkoutOptions?.productId || '');
      ...
      <RadioGroup
        ...
        <div className="flex items-center gap-2">
          <Label
            ...
            <RadioGroupItem id="pay-type-product-id" value={CheckoutTypes.ProductID} />
            Product ID
          </Label>
          <Label
            htmlFor="pay-type-charge-id"
            className="flex cursor-pointer items-center gap-2 rounded-md border p-2 [&:has(:checked)]:bg-muted">
            <RadioGroupItem id="pay-type-charge-id" value={CheckoutTypes.ChargeID} />
            Charge ID
          </Label>
        </div>
      </RadioGroup>
