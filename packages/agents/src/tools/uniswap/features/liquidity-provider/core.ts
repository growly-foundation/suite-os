...
async function findBestLiquidityPairs(
  ...
  const stablecoins = sortedTokens.filter(t => ['USDC', 'USDT', 'DAI', 'BUSD'].includes(t.symbol));
...
function formatLiquidityPlanResponse(plan: LiquidityPlan): string {
  ...
  const { tokenA, tokenB, tokenAmountA, tokenAmountB, expectedAPR, uniswapLink } =
    liquidityRecommendation;
...
export const analyzeAndSuggestLiquidityPools: ToolFn = () => {
