...
function validateBuild() {
  try {
    ...
    const tarballContentsStart = lines.findIndex(line => TARBALL_CONTENTS_START_REGEX.test(line));
    const tarballContentsEnd = lines.findIndex(line => TARBALL_CONTENTS_END_REGEX.test(line));
    ...
    if (missingFiles.length > 0) {
      console.error('Failed to find the following files:\n', missingFiles.join('\n'));
