...
describe('Token explore', () => {
  ...
  it('should load token leaderboard', () => {
    ...
    cy.get(getTestSelector('token-table-row-NATIVE'))
      ...
      .should('include.text', '$');
    cy.get(getTestSelector('token-table-row-NATIVE'))
      .find(getTestSelector('price-cell'))
      .should('include.text', '$');
    cy.get(getTestSelector('token-table-row-NATIVE'))
      .find(getTestSelector('fdv-cell'))
      .should('include.text', '$');
  ...
  it('should update when global network changed', () => {
    cy.visit('/explore/tokens/ethereum');
    cy.get(getTestSelector('tokens-network-filter-selected'))
      .invoke('attr', 'alt')
      .should('eq', `Ethereum logo`);
    ...
    cy.get(getTestSelector('tokens-network-filter-selected'))
      .invoke('attr', 'alt')
      .should('eq', `Optimism logo`);
    cy.get(getTestSelector('token-table-row-NATIVE'))
      .find(getTestSelector('name-cell'))
      .should('include.text', 'Ethereum');
  });
