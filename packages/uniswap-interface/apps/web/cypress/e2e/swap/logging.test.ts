...
describe('swap flow logging', () => {
  it('completes two swaps and verifies the TTS logging for the first, plus all intermediate steps along the way', () => {
    ...
    // Verify Swap Quote
    cy.waitForAmplitudeEvent(SwapEventName.SWAP_QUOTE_FETCH, ['time_to_first_quote_request']).then(
      (event: any) => {
        ...
        cy.wrap(event.event_properties.time_to_first_quote_request_since_first_input).should(
          'be.a',
          'number'
        );
        cy.wrap(event.event_properties.time_to_first_quote_request_since_first_input).should(
