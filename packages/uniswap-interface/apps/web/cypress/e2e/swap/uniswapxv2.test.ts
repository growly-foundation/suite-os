...
describe('UniswapX v2', () => {
  ...
  describe('ETH Input', () => {
    ...
    setupHardhat(async hardhat => {
      await hardhat.fund(
        hardhat.wallet,
        CurrencyAmount.fromRawAmount(nativeOnChain(UniverseChainId.Mainnet), 5e18)
      );
      await hardhat.mine();
    });
    ...
    it('can swap using uniswapX with ETH as input', () => {
      ...
      // Verify fill
      cy.intercept(OrderStatusEndpoint, { fixture: 'uniswapx-v2/filledStatusResponse.json' }).as(
        'orderStatusFilled'
      );
  ...
  describe('Orders', () => {
    ...
    beforeEach(() => {
      ...
      cy.intercept(Xv2OrderSubmissionEndpoint, { fixture: 'uniswapx/orderResponse.json' });
      cy.intercept(OrderStatusEndpoint, { fixture: 'uniswapx-v2/openStatusResponse.json' }).as(
        'orderStatusOpen'
      );
    ...
    it('renders proper view if uniswapx order has insufficient funds', () => {
      ...
      cy.intercept(OrderStatusEndpoint, {
        fixture: 'uniswapx-v2/insufficientFundsStatusResponse.json',
      }).as('orderStatusInsufficient');
    ...
    describe('activity history', () => {
      ...
      it('balances should refetch after uniswapx swap is filled', () => {
        cy.interceptGraphqlOperation('PortfolioBalances', 'portfolio_balances.json').as(
          'PortfolioBalances'
        );
