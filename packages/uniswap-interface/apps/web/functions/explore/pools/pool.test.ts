const pools = [
  {
    ...
    name: 'WBTC/WETH',
    image:
      'http://127.0.0.1:3000/api/image/pools/ethereum/0xCBCdF9626bC03E24f779434178A73a0B4bad62eD',
  },
  {
    ...
    name: 'DAI/MKR',
    image:
      'http://127.0.0.1:3000/api/image/pools/ethereum/0x517F9dD285e75b599234F7221227339478d0FcC8',
  },
  {
    ...
    name: 'USDC/WLD',
    image:
      'http://127.0.0.1:3000/api/image/pools/optimism/0xD1F1baD4c9E6c44DeC1e9bF3B94902205c5Cd6C3',
  },
];
...
test.each(pools)('should inject metadata for valid pools', async pool => {
  ...
  expect(body).toContain(
    `<meta property="og:title" content="${pool.name} on Uniswap" data-rh="true">`
  );
  ...
  expect(body).toContain(
    `<meta property="og:image:alt" content="${pool.name} on Uniswap" data-rh="true">`
  );
  expect(body).toContain(
    `<meta property="twitter:card" content="summary_large_image" data-rh="true">`
  );
  expect(body).toContain(
    `<meta property="twitter:title" content="${pool.name} on Uniswap" data-rh="true">`
  );
  expect(body).toContain(`<meta property="twitter:image" content="${pool.image}" data-rh="true">`);
  expect(body).toContain(
    `<meta property="twitter:image:alt" content="${pool.name} on Uniswap" data-rh="true">`
  );
});
