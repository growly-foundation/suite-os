...
export function* handleSignatureStep({
  setCurrentStep,
  step,
  ignoreInterrupt,
  account,
...
export function* handleOnChainStep<T extends OnChainTransactionStep>(
  params: HandleOnChainStepParams<T>
) {
  const {
    account,
    step,
  ...
  if (duplicativeTx) {
    if (duplicativeTx.status === TransactionStatus.Confirmed) {
      addTransactionBreadcrumb({
        step,
        data: { duplicativeTx: true, hash: duplicativeTx.hash },
        status: 'complete',
      });
...
function* handleOnChainConfirmation(
  params: HandleOnChainStepParams,
  hash: string
...
function* recoverTransactionFromHash(
  hash: `0x${string}`,
  step: OnChainTransactionStep
): SagaGenerator<VitalTxFields> {
...
interface HandlePermitStepParams
  extends Omit<HandleOnChainStepParams<Permit2TransactionStep>, 'info'> {}
...
function* findDuplicativeTx(
  ...
  const transactionMap =
    (yield* select((state: InterfaceState) => state.localWebTransactions[chainId])) ?? {};
  const transactionsForAccount = Object.values(transactionMap).filter(tx =>
    isSameAddress(tx.from, account.address)
  );
...
function* waitForTransaction(hash: string, step: TransactionStep) {
  while (true) {
    const { payload } = yield* take<ReturnType<typeof finalizeTransaction>>(
      finalizeTransaction.type
    );
