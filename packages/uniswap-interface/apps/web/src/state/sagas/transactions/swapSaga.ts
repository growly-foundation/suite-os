...
export function useSwapCallback(): SwapCallback {
  ...
  return useCallback(
    (args: SwapCallbackParams) => {
      ...
      const isBatched =
        isClassicSwap && swapTxContext.txRequests && swapTxContext.txRequests.length > 1;
      const includedPermitTransactionStep =
        isClassicSwap && swapTxContext.permit?.method === PermitMethod.Transaction;
