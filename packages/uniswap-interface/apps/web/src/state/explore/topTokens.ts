  ...
  return useMemo(() => {
    ...
    if (lowercaseFilterString) {
      returnTokens = returnTokens?.filter(token => {
        const addressIncludesFilterString = token?.address
          ?.toLowerCase()
          .includes(lowercaseFilterString);
        const projectNameIncludesFilterString = token?.project?.name
          ?.toLowerCase()
          .includes(lowercaseFilterString);
        const nameIncludesFilterString = token?.name?.toLowerCase().includes(lowercaseFilterString);
        const symbolIncludesFilterString = token?.symbol
          ?.toLowerCase()
          .includes(lowercaseFilterString);
        return (
  ...
  const tokenSortRank = useMemo(
    () =>
      sortedTokenStats?.reduce((acc, cur, i) => {
        ...
        const currCurrencyId = buildCurrencyId(
          fromGraphQLChain(cur.chain) ?? UniverseChainId.Mainnet,
          cur.address
        );
        return {
          ...acc,
  ...
  const sparklines = useMemo(() => {
    ...
    unwrappedTokens?.forEach(current => {
      if (current !== undefined) {
        const address =
          current?.address === NATIVE_CHAIN_ID ? NATIVE_CHAIN_ID : current?.address?.toLowerCase();
