    ...
    ({
      ...
      if (newOutputHasTax && swapState.independentField === CurrencyField.INPUT) {
        ...
        setSwapState(prev => ({
          ...prev,
          independentField:
            prev.independentField === CurrencyField.INPUT
...
export function serializeSwapStateToURLParameters(
  ...
  return (
    ...
    createBaseSwapURLParams({
      chainId,
      outputChainId:
        outputCurrency?.chainId !== inputCurrency?.chainId ? outputCurrency?.chainId : undefined,
...
export function queryParametersToCurrencyState(parsedQs: ParsedQs): SerializedCurrencyState {
  ...
  const inputCurrencyId = parseCurrencyFromURLParameter(
    parsedQs.inputCurrency ?? parsedQs.inputcurrency
  );
  const parsedOutputCurrencyId = parseCurrencyFromURLParameter(
    parsedQs.outputCurrency ?? parsedQs.outputcurrency
  );
  const outputCurrencyId =
    parsedOutputCurrencyId === inputCurrencyId && outputChainId === chainId
      ? undefined
      : parsedOutputCurrencyId;
...
export function useInitialCurrencyState(): {
  ...
  const supportedChainId =
    useSupportedChainId(parsedCurrencyState.chainId ?? defaultChainId) ?? UniverseChainId.Mainnet;
  ...
  const initialTypedValue =
    initialInputCurrency || initialOutputCurrency ? parsedCurrencyState.value : undefined;
