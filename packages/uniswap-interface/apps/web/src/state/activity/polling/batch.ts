...
type PendingBatchDetails = Required<Pick<PendingTransactionDetails, 'batchInfo'>> &
    ...
    for (const transaction of pendingBatchTransactions) {
      try {
        ...
        if (result?.status === 200) {
          ...
          const updatedStatus =
            receipt.status === '0x1' ? TransactionStatus.Confirmed : TransactionStatus.Failed;
        ...
        FAILURE_COUNT_MAP[transaction.batchInfo.batchId] =
          (FAILURE_COUNT_MAP[transaction.batchInfo.batchId] ?? 0) + 1;
        if (FAILURE_COUNT_MAP[transaction.batchInfo.batchId] >= FAILURE_COUNT_THRESHOLD) {
          ...
          logger.error(error, {
            tags: { file: 'batch.ts', function: 'pollAllPendingBatches' },
            extra: { connectorId },
          });
...
function getCallsStatus(params: {
