...
const SWAP_STATUS_TO_FINALIZED_STATUS: Partial<
  Record<SwapStatus, ConfirmedTransactionDetails['status']>
> = {
    ...
    async (txs: BridgeTransactionDetailsWithChainId[]) => {
      ...
      for (const [chainId, txHashes] of txHashesByChain.entries()) {
        try {
          ...
          logger.debug(
            'bridge',
            `usePollPendingBridgeTransactions`,
            'failed to fetch from swapStatus',
            {
              chainId,
              txHashes,
            }
