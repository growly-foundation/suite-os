...
describe('hooks', () => {
  describe('#tryParsePrice', () => {
    ...
    it('should return a price when decimals are the same', () => {
      ...
      expect(tryParsePrice(baseToken, quoteToken, '20.123456789')?.toSignificant(6)).toEqual(
        '20.1235'
      );
      expect(tryParsePrice(baseToken, quoteToken, '0.123456789')?.toSignificant(6)).toEqual(
        '0.123457'
      );
      expect(tryParsePrice(baseToken, quoteToken, '.123456789')?.toSignificant(6)).toEqual(
        '0.123457'
      );
      expect(tryParsePrice(baseToken, quoteToken, '20.')?.toSignificant(6)).toEqual('20');
      expect(
    ...
    it('should parse very small prices', () => {
      ...
      const baseToken = new Token(1, '0x6b175474e89094c44da98b954eedeac495271d0f', 6);
      const quoteToken = new Token(1, '0x1b175474e89094c44da98b954eedeac495271d0f', 6);
      expect(tryParsePrice(baseToken, quoteToken, value)?.toSignificant(6)).toEqual(
        '0.00000000506383'
      );
    });
  });
