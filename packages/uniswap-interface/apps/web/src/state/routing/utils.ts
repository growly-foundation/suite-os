...
function toUnsignedV3DutchOrderInfo(
  orderInfoJSON: UnsignedV3DutchOrderInfoJSON
): UnsignedV3DutchOrderInfo {
  ...
  return {
    ...
    input: {
      ...
      curve: {
        ...
        relativeAmounts: input.curve.relativeAmounts.map(amount =>
...
function toUnsignedPriorityOrderInfo(
  orderInfoJSON: UnsignedPriorityOrderInfoJSON
): UnsignedPriorityOrderInfo {
...
export async function transformQuoteToTrade(
  ...
  const classicTrade = new ClassicTrade({
    v2Routes:
      routes
        ?.filter(
          (r): r is RouteResult & { routev2: NonNullable<RouteResult['routev2']> } =>
            r.routev2 !== null
        )
    ...
    v3Routes:
      routes
        ?.filter(
          (r): r is RouteResult & { routev3: NonNullable<RouteResult['routev3']> } =>
            r.routev3 !== null
        )
