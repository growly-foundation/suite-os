  ...
  const loadMore = useCallback(
    ({ onComplete }: { onComplete?: () => void }) => {
      ...
      fetchMoreV4({
        variables: {
          cursor:
            dataV4?.token?.v4Transactions?.[dataV4.token?.v4Transactions.length - 1]?.timestamp,
        },
        updateQuery: (prev, { fetchMoreResult }) => {
          ...
          const mergedData = {
            token: {
              ...
              v4Transactions: [
                ...(prev.token?.v4Transactions ?? []),
                ...(fetchMoreResult.token?.v4Transactions ?? []),
              ],
            },
          };
          loadingMoreV4.current = false;
      ...
      fetchMoreV3({
        variables: {
          cursor:
            dataV3?.token?.v3Transactions?.[dataV3.token?.v3Transactions.length - 1]?.timestamp,
        },
        updateQuery: (prev, { fetchMoreResult }) => {
          if (!fetchMoreResult) {
      ...
      fetchMoreV2({
        variables: {
          cursor:
            dataV2?.token?.v2Transactions?.[dataV2.token?.v2Transactions.length - 1]?.timestamp,
        },
        updateQuery: (prev, { fetchMoreResult }) => {
          if (!fetchMoreResult) {
