    ...
    const recentTransactions = data.portfolios[0].assetActivities
      .filter(
        activity =>
          ...
          (activity.details as TransactionDetails).assetChanges[0]?.__typename ===
            'TokenTransfer' &&
          ((activity.details as TransactionDetails).assetChanges as TokenTransfer[])[0].asset
            .project &&
          !((activity.details as TransactionDetails).assetChanges as TokenTransfer[])[0].asset
            .project?.isSpam
      )
      ...
      .map(
        activity => ((activity!.details as TransactionDetails).assetChanges as TokenTransfer[])[0]
      );
