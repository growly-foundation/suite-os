...
const ButtonContent = ({ compact }: { compact: boolean }) => {
  ...
  const isInvalidSlippage =
    validateUserSlippageTolerance(userSlippageTolerance) !== SlippageValidationResult.Valid;
    ...
    <Flex
      ...
      <ThemedText.Caption color={isInvalidSlippage ? 'accentWarning' : 'neutral2'}>
        {compact ? (
          ...
          <Trans
            i18nKey="swap.slippage.amt"
            values={{ amt: formatPercent(userSlippageTolerance) }}
          />
        )}
      </ThemedText.Caption>
      <Settings height="24px" width="24px" fill="neutral2" />
