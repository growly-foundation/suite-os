  ...
  test.describe('Company menu', () => {
    ...
    test('contains appropriate sections and links', async ({ page }) => {
      ...
      for (const section of companyMenu) {
        await expect(
          page.getByTestId(TestID.NavCompanyDropdown).getByText(section.label)
        ).toBeVisible();
    ...
    test('includes social media links', async ({ page }) => {
      ...
      for (const link of socialMediaLinks) {
        await expect(
          page.getByTestId(TestID.NavCompanyDropdown).locator(`a[href='${link}']`)
        ).toBeVisible();
      }
    });
  });
...
test.describe('Mobile navigation', () => {
  ...
  test('contains appropriate sections and links', async ({ page }) => {
    ...
    for (const section of companyMenu) {
      ...
      for (const item of section.items) {
        await expect(drawer.locator(`a:has-text("${item.label}")`)).toHaveAttribute(
          'href',
          item.href
        );
      }
  ...
  test('includes social media links', async ({ page }) => {
    for (const link of socialMediaLinks) {
      await expect(
        page.getByTestId(TestID.CompanyMenuMobileDrawer).locator(`a[href='${link}']`)
      ).toBeVisible();
    }
  });
});
