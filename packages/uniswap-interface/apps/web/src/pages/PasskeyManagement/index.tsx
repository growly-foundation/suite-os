...
export function handleRouteToPasskeyManagement({
  ...
  return function handlePasskeyEffect() {
    ...
    if (
      !account.address &&
      !passkeyConnectionAttemptedRef.current &&
      !accountDrawerHasBeenOpenedRef.current
    ) {
...
export default function PasskeyManagement() {
  ...
  const { closeModal: closeRecentlyConnectedModal } = useModalState(
    ModalName.RecentlyConnectedModal
  );
