...
export function RemoveLiquidityV2WithTokenRedirects() {
  ...
  const [currencyA, currencyB] = [
    useCurrency(currencyAddressA, chainId),
    useCurrency(currencyAddressB, chainId),
  ];
  ...
  if (pair) {
    return (
      <Navigate
        to={`/positions/v2/${toGraphQLChain(chainId).toLowerCase()}/${pair.liquidityToken.address}`}
        replace
      />
    );
  }
