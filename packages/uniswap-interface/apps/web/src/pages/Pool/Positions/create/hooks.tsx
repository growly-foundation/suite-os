...
export function useDerivedPositionInfo(state: PositionState): CreatePositionInfo {
  ...
  const v3Pool =
    protocolVersion === ProtocolVersion.V3 ? (v3PoolResult[1] ?? undefined) : undefined;
  ...
  const { price: defaultInitialPrice, isLoading: isDefaultInitialPriceLoading } =
...
export function useTokenBalanceWithBuffer(
...
export function useDepositInfo(state: UseDepositInfoProps): DepositInfo {
  ...
  const dependentAmount: CurrencyAmount<Currency> | undefined = useMemo(() => {
    ...
    return (
      dependentToken && CurrencyAmount.fromRawAmount(dependentToken, dependentTokenAmount.quotient)
    );
  }, [
    state,
    protocolVersion,
  ...
  const dependentField =
    exactField === PositionField.TOKEN0 ? PositionField.TOKEN1 : PositionField.TOKEN0;

  const parsedAmounts: { [field in PositionField]: CurrencyAmount<Currency> | undefined } =
    useMemo(() => {
      return {
        [PositionField.TOKEN0]:
          exactField === PositionField.TOKEN0 ? independentAmount : dependentAmount,
        [PositionField.TOKEN1]:
          exactField === PositionField.TOKEN0 ? dependentAmount : independentAmount,
      };
    }, [dependentAmount, independentAmount, exactField]);
  const { [PositionField.TOKEN0]: currency0Amount, [PositionField.TOKEN1]: currency1Amount } =
    parsedAmounts;
...
export function useInitialPoolInputs() {
  ...
  const { currencyAddressA, currencyAddressB } = useMemo(() => {
    const currencyAddressA = parseCurrencyFromURLParameter(
      parsedQs.currencyA ?? parsedQs.currencya
    );
    const parsedCurrencyAddressB = parseCurrencyFromURLParameter(
      parsedQs.currencyB ?? parsedQs.currencyb
    );
    const currencyAddressB =
      parsedCurrencyAddressB === currencyAddressA ? undefined : parsedCurrencyAddressB;

    // prevent weth + eth
    const isETHOrWETHA =
      checkIsNative(currencyAddressA) ||
      currencyAddressA === WRAPPED_NATIVE_CURRENCY[supportedChainId]?.address;
    const isETHOrWETHB =
      checkIsNative(currencyAddressB) ||
      currencyAddressB === WRAPPED_NATIVE_CURRENCY[supportedChainId]?.address;

    return {
      currencyAddressA,
      currencyAddressB:
        currencyAddressB && !(isETHOrWETHA && isETHOrWETHB) ? currencyAddressB : undefined,
    };
  }, [
    parsedQs.currencyA,
    parsedQs.currencyB,
