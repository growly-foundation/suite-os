...
export function CreatePositionContextProvider({
  ...
  const [dynamicFeeTierSpeedbumpData, setDynamicFeeTierSpeedbumpData] =
    useState<DynamicFeeTierSpeedbumpData>({
      open: false,
...
export function PriceRangeContextProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <PriceRangeContext.Provider
      value={{ reset, priceRangeState, setPriceRangeState, derivedPriceRangeInfo }}>
      {children}
    </PriceRangeContext.Provider>
  );
}
...
export function CreateTxContextProvider({ children }: { children: React.ReactNode }) {
  ...
  const validatedValue = useMemo(() => {
    ...
    return {
      ...
      dependentAmount:
        createError && dependentAmountFallback
