...
function LimitForm({ onCurrencyChange }: LimitFormProps) {
  ...
  const { currencyBalances, parsedAmounts, parsedLimitPrice, limitOrderTrade, marketPrice } =
    derivedLimitInfo;
  ...
  const displayDelegationMismatchUI =
    isPermitMismatchUxEnabled && isDelegationMismatch?.hasMismatch;
    ...
    const marketPriceString = formatCurrencyAmountWithoutUserLocale({
      amount: (() => {
        if (limitState.limitPriceInverted) {
          return marketPrice
            .invert()
            .quote(CurrencyAmount.fromRawAmount(outputCurrency, 10 ** outputCurrency.decimals));
        } else {
    ...
    (type: keyof CurrencyState, newCurrency: Currency, isResettingWETHAfterWrap?: boolean) => {
      ...
      if (otherCurrency && (newCurrency.isNative || otherCurrency.isNative)) {
        ...
        if (nativeCurrency.wrapped.equals(nonNativeCurrency)) {
          newCurrencyState[type === 'inputCurrency' ? 'outputCurrency' : 'inputCurrency'] =
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (nativeCurrency.wrapped.equals(nonNativeCurrency)) {
        onSelectCurrency(
          'outputCurrency',
          getChainInfo(chainId).spotPriceStablecoinAmount.currency
        );
      }
    }
  ...
  return (
    <Column gap="xs">
      ...
      {displayDelegationMismatchUI ? (
        <LimitOrdersNotSupportedBanner
          onMoreDetails={() => setDisplayDelegationMismatchModal(true)}
        />
      ) : (
        <SubmitOrderButton
              ...
              <Trans
                ...
                components={{
                  link: (
                    <Anchor
                      textDecorationLine="none"
                      href={uniswapUrls.helpArticleUrls.limitsFailure}
                      target="_blank">
                      <LearnMore>
                        <Trans i18nKey="common.button.learn" />
                      </LearnMore>
...
function SubmitOrderButton({
  ...
  const isDisabled =
    (!isLimitSupportedChain || hasInsufficientFunds || !!limitPriceError || !trade) &&
    account.isConnected;
