...
function BuyFormInner({ disabled, initialCurrency }: BuyFormProps) {
  ...
  const { amountOut, amountOutLoading, supportedTokens, countryOptionsResult, error } =
    derivedBuyFormInfo;
  ...
  useEffect(() => {
    ...
    if (initialCurrency) {
      ...
      if (quoteCurrencyCode) {
        ...
        supportedToken = supportedTokens?.find(
          meldToken => meldToken.meldCurrencyCode === quoteCurrencyCode
        );
      }
    }
    ...
    const supportedNativeToken = supportedTokens?.find(meldToken => {
      return (
        meldToken.currencyInfo?.currency.chainId === account.chainId &&
        meldToken.currencyInfo?.currency.isNative
      );
    });
      ...
      {countryOptionsResult?.supportedCountries && (
        <CountryListModal
          onSelectCountry={selectedCountry =>
            setBuyFormState(state => ({ ...state, selectedCountry }))
          }
