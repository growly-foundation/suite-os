...
export function useConnectorWithId(
  id: ConnectorID,
  options?: { shouldThrow: true }
): Connector | undefined {
  ...
  return useMemo(
    () =>
      options?.shouldThrow
        ? getConnectorWithId(connectors, id, options)
        : getConnectorWithId(connectors, id),
...
function getInjectedConnectors(
  ...
  const fallbackInjector = getConnectorWithId(
    connectors,
    CONNECTION_PROVIDER_IDS.INJECTED_CONNECTOR_ID,
    {
      shouldThrow: true,
    }
  );
...
export function useOrderedConnections(options?: {
  ...
  return useMemo(() => {
    const { injectedConnectors: injectedConnectorsBase, isCoinbaseWalletBrowser } =
      getInjectedConnectors(connectors, isEmbeddedWalletEnabled);
    ...
    if (isPlaywrightEnv()) {
      const mockConnector = getConnectorWithId(
        connectors,
        CONNECTION_PROVIDER_IDS.MOCK_CONNECTOR_ID,
        SHOULD_THROW
      );
    ...
    if (showSecondaryConnectors) {
      ...
      orderedConnectors.push(
        ...secondaryConnectors.filter(c => c.id !== recentConnectorId || isMobileWeb)
      );
    } else {
