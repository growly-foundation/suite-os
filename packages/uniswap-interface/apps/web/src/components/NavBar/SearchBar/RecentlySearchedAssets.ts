...
export function useRecentlySearchedAssets(): {
  data?: InterfaceRemoteSearchHistoryItem[];
  loading: boolean;
} {
  ...
  const shortenedHistory = useMemo(
    () =>
      history
        .filter(item => item.type !== SearchResultType.NFTCollection)
  ...
  const { data: queryData, loading } = useRecentlySearchedAssetsQuery({
    variables: {
      collectionAddresses: shortenedHistory
        .filter(isNFTCollectionSearchResult)
