...
const TableCell = memo(
  ({ cell, colors }: { cell: Cell<RowData, unknown>; colors: UseSporeColorsReturn }) => {
    return (
...
const TableBody = forwardRef<HTMLDivElement, TableBodyProps<RowData>>(
  ({ table, loading, error }, ref) => {
    ...
    <TableSizeProvider value={tableSize}>
      <ScrollSync horizontal>
        <TableContainer
          ...
          {hasPinnedColumns && (
            <TableScrollMask zIndex={zIndexes.default} borderBottomRightRadius="$rounded20" />
          )}
          <ScrollSyncPane group="table-sync">
