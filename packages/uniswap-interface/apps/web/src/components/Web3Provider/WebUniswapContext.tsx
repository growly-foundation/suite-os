...
import {
  useHasMismatchCallback,
  useShowMismatchToast,
} from 'state/walletCapabilities/hooks/useMismatchAccount';
...
function WebUniswapProviderInner({ children }: PropsWithChildren) {
  ...
  const navigateToReceive = useCallback(
    () => setReceiveModalState(ModalState.QR_CODE),
  ...
  const navigateToNftCollection = useCallback(
    (args: { collectionAddress: Address; chainId: UniverseChainId }) => {
      window.open(
  ...
  const getCanBatchTransactions = useEvent((chainId?: UniverseChainId | undefined) => {
    return Boolean(
      isBatchedSwapsFlagEnabled &&
        isOneClickSwapSettingEnabled &&
        chainId &&
        isAtomicBatchingSupportedByChain(chainId)
    );
  });
...
const MismatchContextWrapper = React.memo(function MismatchContextWrapper({
  ...
  const { chains, defaultChainId, isTestnetModeEnabled } = useEnabledChainsWithConnector(
    account.connector
  );
  return (
...
function useAccountChainIdEffect() {
  const currentChainId = useSelector(
    (state: { delegation: DelegatedState }) => state.delegation.activeChainId
  );
