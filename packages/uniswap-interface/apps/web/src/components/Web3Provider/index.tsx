...
import {
  UNISWAP_EXTENSION_CONNECTOR_NAME,
...
export function Web3ProviderUpdater() {
  ...
  const networkProvider =
    isSupportedChain && account.chainId ? RPC_PROVIDERS[account.chainId] : undefined;
  ...
  const previousConnectedChainId = usePrevious(
    account.isConnected ? accountWagmiChainId : undefined
  );
  useEffect(() => {
    const chainChanged =
      previousConnectedChainId && previousConnectedChainId !== accountWagmiChainId;
    if (chainChanged) {
...
const WalletCapabilitiesEffects: React.FC = isTestEnv()
  ? () => null
  : WalletCapabilitiesEffectsInner;
