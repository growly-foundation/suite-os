...
function SwapFeeTooltipContent({ hasFee }: { hasFee: boolean }) {
  const message = hasFee ? (
    <Trans i18nKey="swap.fees.experience" />
  ) : (
...
function MinimumOutputTooltipContent({ amount }: { amount: CurrencyAmount<Currency> }) {
  ...
  return (
    <BaseTooltipContent url="https://support.uniswap.org/hc/en-us/articles/8643794102669-Price-Impact-vs-Price-Slippage">
      <Trans
        i18nKey="swap.minPriceSlip.revert"
        values={{ amount: `${formattedAmount} ${amount.currency.symbol}` }}
      />
    </BaseTooltipContent>
  );
}
...
function ColoredPercentRow({ percent }: { percent: Percent }) {
  ...
  return (
    <ColorWrapper textColor={getPriceImpactColor(positivePercent)}>{formattedPercent}</ColorWrapper>
  );
}
  ...
  switch (type) {
    ...
    case SwapLineItemType.PRICE_IMPACT:
      ...
      if (
        isUniswapX ||
        !priceImpact ||
        (isPreview && isUniswapXTradeType(lastSubmittableFillType))
      ) {
        return undefined;
      }
      return {
    ...
    case SwapLineItemType.MAXIMUM_INPUT:
      ...
      return {
        ...
        Value: () => (
          <CurrencyAmountRow amount={trade.maximumAmountIn(allowedSlippage ?? new Percent(0))} />
        ),
        loaderWidth: 70,
      };
    case SwapLineItemType.MINIMUM_OUTPUT:
      return {
        ...
        TooltipBody: () => (
          <MinimumOutputTooltipContent
            amount={trade.minimumAmountOut(allowedSlippage ?? new Percent(0))}
          />
        ),
        Value: () => (
          <CurrencyAmountRow amount={trade.minimumAmountOut(allowedSlippage ?? new Percent(0))} />
        ),
        loaderWidth: 70,
      };
...
function getFOTLineItem({
  ...
  trade,
  t,
}: SwapLineItemProps & { t: TFunction }): LineItemData | undefined {
  ...
  return {
    Label: () => (
      <>
        {tokenSymbol
          ? t('swap.details.feeOnTransfer', { tokenSymbol })
          : t('swap.details.feeOnTransfer.default')}
      </>
    ),
