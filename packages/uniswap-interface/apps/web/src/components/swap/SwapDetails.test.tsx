...
describe('SwapDetails.tsx', () => {
  it('matches base snapshot, test trade exact input', () => {
    ...
    const tradeMinAmount = TEST_TRADE_EXACT_INPUT.minimumAmountOut(
      TEST_ALLOWED_SLIPPAGE ?? new Percent(0)
    );
    ...
    expect(
      screen.getByText('The impact your trade has on the market price of this pool.')
    ).toBeInTheDocument();
    expect(
      screen.getByText('The maximum price movement before your transaction will revert.')
    ).toBeInTheDocument();
  });
