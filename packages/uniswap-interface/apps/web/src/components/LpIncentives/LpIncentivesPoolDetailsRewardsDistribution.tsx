...
export const LpIncentivesPoolDetailsRewardsDistribution = ({
  ...
  const distributedRewardsAmount = CurrencyAmount.fromRawAmount(
    LP_INCENTIVES_REWARD_TOKEN,
    distributedRewardsRaw
  );
  const totalRewardAllocationAmount = CurrencyAmount.fromRawAmount(
    LP_INCENTIVES_REWARD_TOKEN,
    totalRewardAllocationRaw
  );
  ...
  const dateRange = formatDateRange(
    rewardsCampaign.startTimestamp,
    rewardsCampaign.endTimestamp,
    currentLanguage
  );
      ...
      <Flex flex={1} gap="$gap8" minWidth={180} $md={{ minWidth: 150 }}>
        ...
        <Flex row justifyContent="space-between" alignItems="flex-end">
          <Text variant="body3" color="$neutral1">
            {formattedDistributedToken} {LP_INCENTIVES_REWARD_TOKEN.symbol} (
            {formattedDistributedFiat})
          </Text>
