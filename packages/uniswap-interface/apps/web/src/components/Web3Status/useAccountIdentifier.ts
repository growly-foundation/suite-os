...
export function useAccountIdentifier() {
  const [recentAccountIdentifierMap, updateRecentAccountIdentifierMap] = useAtom(
    recentAccountIdentifierMapAtom
  );
  ...
  useEffect(() => {
    ...
    updateRecentAccountIdentifierMap(prev => {
      ...
      return {
        ...prev,
        [account.address as string]: updatedIdentifiers,
        recent: updatedIdentifiers,
      };
    });
  ...
  const { unitag, ensName } =
    (account.address
      ? recentAccountIdentifierMap[account.address]
      : recentAccountIdentifierMap['recent']) ?? {};
