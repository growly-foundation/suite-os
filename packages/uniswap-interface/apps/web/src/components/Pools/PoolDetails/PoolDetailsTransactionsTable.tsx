...
function comparePoolTokens(tokenA: PoolTableTransaction['pool']['token0'], tokenB?: Token) {
  if (tokenB?.address === NATIVE_CHAIN_ID) {
    ...
    return (
      chainId &&
      tokenA.id?.toLowerCase() === WRAPPED_NATIVE_CURRENCY[chainId]?.address.toLowerCase()
    );
  }
...
export function PoolDetailsTransactionsTable({
  ...
  const columns = useMemo(() => {
    ...
    return [
      ...
      columnHelper.accessor(
        row => {
          ...
          if (row.type === PoolTableTransactionType.BUY) {
            ...
            color =
              row.type === PoolTableTransactionType.ADD ? '$statusSuccess' : '$statusCritical';
            text =
      ...
      columnHelper.accessor(
        row => (comparePoolTokens(row.pool.token0, token0) ? row.amount0 : row.amount1),
        {
          id: 'input-amount',
      ...
      columnHelper.accessor(
        row => (comparePoolTokens(row.pool.token0, token0) ? row.amount1 : row.amount0),
        {
          id: 'output-amount',
      ...
      columnHelper.accessor(row => row.maker, {
        ...
        cell: makerAddress => (
          <Cell loading={showLoadingSkeleton} justifyContent="flex-end" grow>
            <StyledExternalLink
              href={getExplorerLink(chainId, makerAddress.getValue?.(), ExplorerDataType.ADDRESS)}>
