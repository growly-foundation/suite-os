...
import {
  usePDPPriceChartData,
  usePDPVolumeChartData,
} from 'components/Pools/PoolDetails/ChartSection/hooks';
import {
...
export default function ChartSection(props: ChartSectionProps) {
  ...
  const loading =
    props.loading || (activeQuery.chartType !== ChartType.LIQUIDITY ? activeQuery?.loading : false);
  ...
  const filteredTimeOptions = useMemo(() => {
    if (activeQuery.chartType === ChartType.PRICE) {
      const filtered = DEFAULT_PILL_TIME_SELECTOR_OPTIONS.filter(
  ...
  const disabledChartOption =
    props.poolData?.protocolVersion === ProtocolVersion.V2 ? ChartType.LIQUIDITY : undefined;
      ...
      {crosshairData => {
        ...
        return (
          <ChartHeader
            value={priceDisplay}
            additionalFields={
              <PriceChartDelta startingPrice={data[0]} endingPrice={displayValue} />
            }
  ...
  return (
    <>
      <ThemedText.BodySmall>
        {t('liquidityPool.chart.tooltip.amount', {
          token: tokenADescriptor,
          amount: displayValue0,
        })}
      </ThemedText.BodySmall>
      <ThemedText.BodySmall>
        {t('liquidityPool.chart.tooltip.amount', {
          token: tokenBDescriptor,
          amount: displayValue1,
        })}
      </ThemedText.BodySmall>
    </>
  );
