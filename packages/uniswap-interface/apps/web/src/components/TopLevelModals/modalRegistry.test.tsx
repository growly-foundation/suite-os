...
jest.mock('components/AccountDrawer/MiniPortfolio/Activity/OffchainActivityModal', () => ({
  ...
  OffchainActivityModal: () => (
    <div data-testid="mock-offchain-activity-modal">Offchain Activity Modal</div>
  ),
}));
...
describe('ModalRegistry', () => {
  describe('ModalRenderer', () => {
    ...
    it('renders AddressClaimModal when modal is open', async () => {
      mocked(useAppSelector).mockReturnValue({
        application: { openModal: { name: ModalName.AddressClaim } },
      });
      await act(async () => {
    ...
    it('renders ConnectedAccountBlocked when modal is open', async () => {
      mocked(useAppSelector).mockReturnValue({
        application: { openModal: { name: ModalName.BlockedAccount } },
      });
      await act(async () => {
    ...
    it('renders modals with custom props', async () => {
      mocked(useAppSelector).mockReturnValue({
        application: { openModal: { name: ModalName.UniWalletConnect } },
      });
      await act(async () => {
    ...
    it('handles modal state from useModalState hook', async () => {
      mocked(useAppSelector).mockReturnValue({
        application: { openModal: { name: ModalName.OffchainActivity } },
      });
      await act(async () => {
