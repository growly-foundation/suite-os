...
function ActivityPopupContent({ activity, onClick, onClose }: ActivityPopupContentProps) {
  ...
  const isBridgeActivity =
    activity.outputChainId && activity.chainId && activity.chainId !== activity.outputChainId;
  return (
    <Flex
      row
...
export function TransactionPopupContent({
  ...
  const { data: activity } = useQuery(
    getTransactionToActivityQueryOptions(transaction, chainId, formatNumber)
  );
  ...
  const onClick = () =>
    window.open(
      getExplorerLink(activity.chainId, activity.hash, ExplorerDataType.TRANSACTION),
      '_blank'
    );
