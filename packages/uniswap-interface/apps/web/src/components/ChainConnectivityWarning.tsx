...
export function ChainConnectivityWarning() {
  ...
  const waitMsBeforeWarning = useMemo(
    () =>
      (chainId ? getChainInfo(chainId)?.blockWaitMsBeforeWarning : undefined) ??
  ...
  const warning = Boolean(
    !!blockTime && machineTime - Number(blockTime) * 1000 > waitMsBeforeWarning
  );
    ...
    <Wrapper>
      ...
      <BodyRow>
        ...
        {info.statusPage !== undefined && (
          <span>
            <Trans
              i18nKey="common.checkNetwork"
              components={{ link: <Link href={info.statusPage || ''} /> }}
            />
          </span>
        )}
      </BodyRow>
