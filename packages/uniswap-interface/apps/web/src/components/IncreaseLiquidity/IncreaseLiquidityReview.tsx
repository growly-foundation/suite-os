...
export function IncreaseLiquidityReview({ onClose }: { onClose: () => void }) {
  ...
  const { currencyAmounts, currencyAmountsUSDValue, deposit0Disabled, deposit1Disabled } =
    derivedIncreaseLiquidityInfo;
  ...
  const { version, poolId, currency0Amount, currency1Amount, feeTier, chainId } =
  ...
  return (
    <Flex gap="$gap12">
      <Flex gap="$gap16" px="$padding16" pt="$padding12">
        <TokenInfo
          currencyAmount={updatedCurrencyAmounts?.TOKEN0}
          currencyUSDAmount={updatedUSDAmounts?.TOKEN0}
        />
        <Text variant="body3" color="$neutral2">
          {t('common.and')}
        </Text>
        <TokenInfo
          currencyAmount={updatedCurrencyAmounts?.TOKEN1}
          currencyUSDAmount={updatedUSDAmounts?.TOKEN1}
        />
      </Flex>
      {currentTransactionStep ? (
