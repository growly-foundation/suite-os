...
function getProviderIcon(provider: AuthenticatorProvider, colors: UseSporeColorsReturn) {
  switch (provider) {
    ...
    case AuthenticatorProvider.Apple:
      return (
        <AppleLogo height={iconSizes.icon20} width={iconSizes.icon20} fill={colors.neutral1.val} />
      );
...
export default function PasskeyMenu({ onClose }: { onClose: () => void }) {
  ...
  const [passkeyMenuModalState, setPasskeyMenuModalState] = useState<
    PasskeyMenuModalState | undefined
  >(undefined);
  const [selectedAuthenticator, setSelectedAuthenticator] = useState<
    AuthenticatorDisplay | undefined
  >(undefined);
  ...
  const { mutate: refreshAuthenticators, isPending: areAuthenticatorsLoading } =
