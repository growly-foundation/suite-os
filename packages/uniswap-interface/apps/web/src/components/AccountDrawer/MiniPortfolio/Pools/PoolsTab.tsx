...
export default function Pools({ account }: { account: string }) {
  ...
  const openPositions = useMemo(
    () => data?.positions.map(parseRestPosition).filter(isPositionInfo),
    [data?.positions]
  );
  ...
  const { visibleOpenPositions, visibleClosedPositions, hiddenPositions } = useMemo(() => {
    ...
    const { visible: visibleOpenPositions, hidden: hiddenOpenPositions } =
      splitByVisibility(openPositions);
    const { visible: visibleClosedPositions, hidden: hiddenClosedPositions } =
      splitByVisibility(closedPositions);
  ...
  return (
    <Trace
      ...
      <Link
        to={positionUrl}
        onClick={handleClick}
        style={{ textDecoration: 'none', display: 'block', margin: '16px' }}>
