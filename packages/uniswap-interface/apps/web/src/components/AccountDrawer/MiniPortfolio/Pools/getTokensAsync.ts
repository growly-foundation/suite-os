...
async function fetchChunk(
  ...
  try {
    ...
    if (
      error.code === INTERNAL_JSON_RPC_ERROR_CODE ||
    ...
    logger.debug('getTokensAsync', 'fetchChunk', 'Error fetching chunk', {
      error,
      extra: { chunk },
    });
    throw error;
...
function tryParseToken(address: string, chainId: UniverseChainId, data: CallResult[]) {
  try {
    ...
    logger.debug('getTokensAsync', 'tryParseToken', 'Failed to parse token', {
      error,
      address,
      chainId,
    });
...
export async function getTokensAsync(
  ...
  formattedAddresses.forEach(
    address =>
      (TokenPromiseCache[buildCurrencyKey(chainId, address)] = calledTokens.then(
        tokenMap => tokenMap[address]
      ))
  );
