  ...
  const [inputRaw, outputRaw] =
    swap.tradeType === TradeType.EXACT_INPUT
      ? [
          ...
          swap.settledOutputCurrencyAmountRaw ?? swap.expectedOutputCurrencyAmountRaw,
        ]
...
async function parseBridge(
  ...
  const inputAmount = tokenIn
    ? formatNumber({
        input: parseFloat(
          CurrencyAmount.fromRawAmount(tokenIn, bridge.inputCurrencyAmountRaw).toSignificant()
        ),
        type: NumberType.TokenNonTx,
      })
    : i18n.t('common.unknown');
  const outputAmount = tokenOut
    ? formatNumber({
        input: parseFloat(
          CurrencyAmount.fromRawAmount(tokenOut, bridge.outputCurrencyAmountRaw).toSignificant()
        ),
        type: NumberType.TokenNonTx,
      })
    : i18n.t('common.unknown');
...
type GenericLiquidityInfo = Omit<
