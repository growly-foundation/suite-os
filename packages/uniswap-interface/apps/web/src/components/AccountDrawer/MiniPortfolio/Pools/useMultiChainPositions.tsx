...
import {
  Call,
  DEFAULT_GAS_LIMIT,
} from 'components/AccountDrawer/MiniPortfolio/Pools/getTokensAsync';
import {
  ...
  const fetchPositionInfo = useCallback(
    async (
      ...
      positionDetails.forEach(details => {
        const tokenA =
          tokens[details.token0] ?? new Token(chainId, details.token0, DEFAULT_ERC20_DECIMALS);
        const tokenB =
          tokens[details.token1] ?? new Token(chainId, details.token1, DEFAULT_ERC20_DECIMALS);
      ...
      return (await multicall.callStatic.multicall(calls)).returnData.reduce(
        (acc: PositionInfo[], result, i) => {
          if (result.success) {
            ...
            acc.push(
              createPositionInfo(account, chainId, positionDetails[i], slot0, ...poolPairs[i])
            );
          } else {
            logger.debug(
