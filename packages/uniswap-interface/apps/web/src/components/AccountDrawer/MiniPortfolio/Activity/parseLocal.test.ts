...
describe('parseLocalActivity', () => {
  ...
  it('Properly uses correct tense of activity title based on tx status', async () => {
    ...
    expect(
      result.current[mockHash('0xswap_exact_input', TransactionStatus.Pending)]?.title
    ).toEqual('Swapping');
    expect(
      result.current[mockHash('0xswap_exact_input', TransactionStatus.Confirmed)]?.title
    ).toEqual('Swapped');
    expect(result.current[mockHash('0xswap_exact_input', TransactionStatus.Failed)]?.title).toEqual(
      'Swap failed'
    );
  });
