...
export function useAllActivities(account: string) {
  ...
  const combinedActivities = useMemo(
    () => combineActivities(localMap, remoteMap ?? {}),
    [localMap, remoteMap]
  );
...
export function usePendingActivity() {
  ...
  const pendingOrdersWithoutLimits = pendingOrders.filter(
    order => order.type !== SignatureType.SIGN_LIMIT
  );

  const hasPendingActivity =
    pendingTransactions.length > 0 || pendingOrdersWithoutLimits.length > 0;
