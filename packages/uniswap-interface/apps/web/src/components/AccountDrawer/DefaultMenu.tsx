...
function DefaultMenu() {
  ...
  const openPortfolioBalanceSettings = useCallback(
    () => setMenu(MenuState.PORTFOLIO_BALANCE),
    [setMenu]
  );
  ...
  const SubMenu = useMemo(() => {
    switch (menu) {
      ...
      case MenuState.POOLS:
        return account.address ? (
          <UniExtensionPoolsMenu account={account.address} onClose={closeLimitsMenu} />
        ) : null;
