...
const getActiveTick = (
  ...
  tickCurrent && feeAmount && tickSpacing
...
function useAllPoolTicks({
  ...
  const {
    ...
    error,
    loading: isLoading,
  } = usePaginatedTickQuery(poolId, version, skipNumber, chainId);
  // TODO: fix typing on usePaginatedTickQuery function to avoid casting to any
  const ticks: Ticks =
    ((data as any)?.v3Pool?.ticks as Ticks) ?? ((data as any)?.v4Pool?.ticks as Ticks);
...
export function usePoolActiveLiquidity({
  ...
  const poolsQueryEnabled = Boolean(
    poolEnabledProtocolVersion(version) && currencyA && currencyB && !skip
  );
