...
function useFeeOnTransferDetectorContract(chainId?: UniverseChainId): FeeOnTransferDetector | null {
  ...
  const contract = useContract<FeeOnTransferDetector>(
    getFeeOnTransferAddress(chainId),
    FOT_DETECTOR_ABI,
    true,
    chainId
  );
...
async function getSwapTaxes(
  ...
  try {
    ...
    logger.warn(
      'useSwapTaxes',
      'getSwapTaxes',
      'Failed to get swap taxes for token(s):',
      addresses,
      e
    );
  }
  ...
  const inputTax =
    (inputTokenAddress ? FEE_CACHE[inputTokenAddress]?.sellTax : ZERO_PERCENT) ?? ZERO_PERCENT;
  const outputTax =
    (outputTokenAddress ? FEE_CACHE[outputTokenAddress]?.buyTax : ZERO_PERCENT) ?? ZERO_PERCENT;
