...
export function useFormatter() {
  ...
  const { convertFiatAmount, conversionRate: localCurrencyConversionRate } =
  ...
  const formatPercentWithLocales = useCallback(
    (percent: Percent | undefined, maxDecimals?: number) =>
      formatPercent(percent, maxDecimals, activeLocale),
    [activeLocale]
  );
