...
function checkSplitFiles() {
  ...
  touchedFiles.forEach(file => {
    ...
    if (isWebFile && !fs.existsSync(`${dirname(__filename)}/${baseFile}.native.${extension}`)) {
      fail(
        `\`${baseFile}.web.${extension}\` must also have a \`${baseFile}.native.${extension}\` file.`
      );
    }

    if (isNativeFile && !fs.existsSync(`${dirname(__filename)}/${baseFile}.web.${extension}`)) {
      fail(
        `\`${baseFile}.native.${extension}\` must also have a \`${baseFile}.web.${extension}\` file.`
      );
    }
...
async function processAddChanges() {
  const updatedTsFiles = danger.git.modified_files
    .concat(danger.git.created_files)
    .filter(
      file => (file.endsWith('.ts') || file.endsWith('.tsx')) && !file.includes('dangerfile.ts')
    );
  ...
  allNonUILinesAdded.forEach(change => {
    if (change.content.includes(`from 'tamagui`)) {
      fail(
        `Please import any tamagui exports via the ui package. Found an import at ${change.content}`
      );
    }
  });
  ...
  if (
    allLinesAdded.some(
      change => change.content.includes('logMessage') || change.content.includes('logger.')
    )
  ) {
...
async function checkCocoaPodsVersion() {
  const updatedPodFileLock = danger.git.modified_files.find(file =>
    file.includes('ios/Podfile.lock')
  );
  if (updatedPodFileLock) {
    ...
    const changedCocoaPodsVersion = changedLines.some(change =>
      change.content.includes('COCOAPODS: ')
    );
...
if (
  ...
  warn(
    'You created or deleted a slice file. Make sure to update the schema and create migration if needed.'
  );
}
