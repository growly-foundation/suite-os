...
const DIST_TAGS_URL = 'https://registry.npmjs.org/-/package/@growly/appstack/dist-tags';
...
export function getNextAlphaVersionNumber({ alpha, latest }) {
  ...
  const nextPatchVersion = [latestSplit[0], latestSplit[1], latestSplit[2] + 1].join('.');
...
export function publishAlphaRelease(nextAlphaVersion) {
  ...
  const widgetsRoot = path.resolve(currentDir, '..');
