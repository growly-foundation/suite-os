...
export const formatPositionsData = (data: ZerionFungiblePosition[]) => {
  const filtered = data.filter(
    (pos) => pos.attributes.value !== null && pos.attributes.value > 1,
  );
  ...
  for (const pos of sorted) {
    const { value, position_type, fungible_info, application_metadata } =
      pos.attributes;

    const chain = pos.relationships.chain.data.id;

    const protocolLine = application_metadata?.name
      ? `via ${application_metadata.name}`
      : '';
