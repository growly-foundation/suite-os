...
class EtherscanLogsProvider(EtherscanBaseProvider):
    ...
    async def _get_logs_internal(
        ...
        """
        Internal method for fetching logs with pagination.

        Consolidates common logic for all three public log fetching methods.
        ...
        # Log operation type based on parameters
        if address and topics:
            ...
            logger.info("Starting logs fetch")
    ...
    async def get_logs_by_address(
        ...
        Raises:
            ...
            Exception: If API request fails
        """
        return await self._get_logs_internal(
            chain_id=chain_id,
            from_block=from_block,
            to_block=to_block,
            address=address,
