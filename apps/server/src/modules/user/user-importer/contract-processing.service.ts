...
export class ContractProcessingService {
  private readonly logger = new Logger(ContractProcessingService.name);
  ...
  async importUniqueAddresses(
    ...
    this.logger.log(`Starting import for contract ${contractAddress} on chain ${chainId}`);
    ...
    // Log processing results
    this.logProcessingResults(contractAddress, chainId, uniqueAddresses.length, stats);
