...
export class EvmChainPlugin {
  ...
  getChainMetadata = async (chainId: TChainId): Promise<TChainMetadataListResponse | undefined> => {
    const metadata = (Files.ChainList.ChainMetadataMap as any)[chainId];
  ...
  getAllChainMetadata = async (): Promise<Record<TChainId, TChainMetadataListResponse>> => {
    try {
      ...
      const data: TChainMetadataListResponse[] = await Promise.all(
        responses.map<TChainMetadataListResponse>(response => response.json() as any)
      );
      const multichainMetadata: Record<TChainId, TChainMetadataListResponse> = {};
  ...
  getTokenMetadataList = async (chainId: TChainId): Promise<TContractTokenMetadata[]> => {
    try {
      ...
      const data: TTokenListResponse[] = await Promise.all(
        responses.map<TTokenListResponse>(response => response.json() as any)
      );
